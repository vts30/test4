import { getPool, closePool } from './core/db';
...
async function initSchema(): Promise<void> {
  ...
  if (!schema) throw new Error('PG_SCHEMA env var is required');
  const pool = getPool();
  try {
    await pool.query(buildSQL(schema));
    console.log(`[perf-recording] Schema initialized in ${schema}`);
  } finally {
    await closePool();
  }
}
...
initSchema().catch((err) => {
  console.error('[perf-recording] Schema init failed:', err.message);
  process.exit(1);
});
